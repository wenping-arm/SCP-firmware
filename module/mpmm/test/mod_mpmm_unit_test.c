...
static struct mod_mpmm_domain_ctx dev_ctx_table[1];
...
void utest_mpmm_element_init_func_success(void)
{
    ...
    status = mpmm_element_init(elem_id, FAKE_MOD_MPMM_ELEM_1, &fake_domain_conf[FAKE_MOD_MPMM_DOM_DEFAULT]);
    TEST_ASSERT_EQUAL(status, FWK_SUCCESS);
}
...
void utest_mpmm_element_init_func_element_count_max_fail(void)
{
    ...
    status = mpmm_element_init(fwk_module_id_mpmm, (MPMM_MAX_NUM_CORES_IN_DOMAIN + 1), NULL);
    TEST_ASSERT_EQUAL(status, FWK_E_PARAM);
}
...
void utest_mpmm_element_init_func_max_threshold_count_fail(void)
{
    ...
    status = mpmm_element_init(elem_id, FAKE_MOD_MPMM_ELEM_1, &fake_domain_conf[FAKE_MOD_MPMM_DOM_MAX_THRESHOLD_COUNT]);
...
void utest_mpmm_element_init_func_threshold_count_not_equal(void)
{
    ...
    status = mpmm_element_init(elem_id, FAKE_MOD_MPMM_ELEM_1, &fake_domain_conf[FAKE_MOD_MPMM_DOM_NUM_THRESHOLD_NOT_EQUAL]);
...
void utest_mpmm_process_notification_func_new_level_success(void)
{
    ...
    //todo: relook at perf_level in config.h so that dont have to manually set current_perf_level
    dev_ctx_table[0].current_perf_level = 3048 * 1000000UL;
    params = (struct mod_pd_power_state_pre_transition_notification_params *)event.params;
...
void utest_mpmm_process_notification_func_domain_ctx_null(void)
{
    int status;
    struct fwk_event event = {0};
    struct fwk_event resp_event = {0};
    fwk_id_t elem_id = FWK_ID_ELEMENT(FWK_MODULE_IDX_MPMM, 1);
    ...
    /*get get_domain_ctx() to return NULL*/
    status = mpmm_process_notification(&event, &resp_event);
    TEST_ASSERT_EQUAL(status, FWK_E_PARAM);
}
...
void utest_mpmm_update_func_success(void)
{
    ...
    //uint32_t adj_max_limit;
    fwk_id_t elem_id = FWK_ID_ELEMENT(FWK_MODULE_IDX_MPMM, 0);
    //struct perf_plugins_perf_update fake_perf_update = {0};

    //fake_perf_update.domain_id = elem_id;
    //fake_perf_update.adj_max_limit = &adj_max_limit;
    ...
    TEST_ASSERT_EQUAL(status, FWK_SUCCESS);
    TEST_ASSERT_EQUAL(*perf_update.adj_max_limit, mpmm_ctx.domain_ctx[0].perf_limit);
}
...
void utest_mpmm_update_func_perf_transition(void)
{
    ...
    //fake_perf_update.adj_max_limit = &adj_max_limit;
    mpmm_ctx.domain_ctx[0].num_cores_online = 1;
    //todo: check other way to modify domain_ctx->perf_limit 
    mpmm_ctx.domain_ctx[0].current_perf_level = 1;
...
void utest_mpmm_report_func_success(void)
{
    ...
    TEST_ASSERT_EQUAL(status, FWK_SUCCESS);
    TEST_ASSERT_EQUAL(mpmm_ctx.domain_ctx[0].current_perf_level, fake_perf_report.level);
...
{
    int status;
    //struct perf_plugins_perf_report =false{0};
...
void utest_mpmm_core_evaluate_threshold_func_success(void)
{
    ...
    core_ctx_table.threshold = 0xA5A5;

    mpmm_core_evaluate_threshold(&dev_ctx_table[0], &core_ctx_table);
    TEST_ASSERT_NOT_EQUAL(core_ctx_table.threshold, 0xA5A5);
}
...
void utest_mpmm_core_evaluate_threshold_func_counter_not_enabled(void)
{
    ...
    mpmm_core_evaluate_threshold(&dev_ctx_table[0], &core_ctx_table);
    TEST_ASSERT_EQUAL(core_ctx_table.threshold, dev_ctx_table[0].domain_config->num_threshold_counters);
}
...
void utest_mpmm_domain_set_thresholds_func_success(void)
{
    ...
    TEST_ASSERT_EQUAL(dev_ctx_table[0].core_ctx[0].mpmm->MPMMCR, (1 << MPMM_MPMMCR_GEAR_POS));
}
...
/*
void utest_mpmm_core_counters_delta_func_no_wraparound(void)
{
    uint32_t counter = 0;
    struct amu_reg amu_aux = {
        .AMEVCNTR_L = 0x01,
    ...
    core_ctx_table.amu_aux = &amu_aux;
    core_ctx_table.cached_counters = &counter;

    //todo: seg fault when return from function
    mpmm_core_counters_delta(&dev_ctx_table[0], &core_ctx_table);
    TEST_ASSERT_EQUAL(core_ctx_table.delta[0], amu_aux.AMEVCNTR_L);
}*/
...
void utest_mpmm_core_threshold_policy_func_highest_gear(void)
{
    ...
    TEST_ASSERT_EQUAL(gear, (dev_ctx_table[0].domain_config->num_threshold_counters - 1));
}

void utest_find_perf_limit_from_pct_func_default_limit(void)
{
    uint32_t perf_limit;
    uint32_t threshold_map = (fake_pct_table[FAKE_PCT_TABLE_NO_THRESHOLD_BITMAP].threshold_perf->threshold_bitmap+1);

    perf_limit = find_perf_limit_from_pct(&fake_pct_table[FAKE_PCT_TABLE_NO_THRESHOLD_BITMAP], threshold_map);
    TEST_ASSERT_EQUAL(perf_limit, fake_pct_table[FAKE_PCT_TABLE_NO_THRESHOLD_BITMAP].default_perf_limit);
}

void utest_mpmm_build_threshold_map_func_default_limit(void)
{
    //uint32_t perf_limit;
    //uint32_t threshold_map = (fake_pct_table[FAKE_PCT_TABLE_NO_THRESHOLD_BITMAP].threshold_perf->threshold_bitmap+1);

    //core_ctx_table.online = 1;
    dev_ctx_table[0].num_cores_online = 2;
    dev_ctx_table[0].core_ctx[0].online = 1;

    mpmm_build_threshold_map(&dev_ctx_table[0]);
    //todo: test threshold in descending order
    //TEST_ASSERT_EQUAL(dev_ctx_table[0].threshold_map, );
}
...
void utest_mpmm_evaluate_perf_limit_func_no_entry(void)
{
    ...
    TEST_ASSERT_EQUAL(perf_limit, DEFAULT_PERF_LIMIT);
}
...
void utest_mpmm_monitor_and_control_func_no_cores_online(void)
{
    ...
    //dev_ctx_table[0].num_cores_online = 2;
    //dev_ctx_table[0].core_ctx[0].online = 1;
    //mpmm_ctx.domain_ctx->domain_config = &fake_domain_conf[FAKE_MOD_MPMM_DOM_NO_PCT_ENTRY];
...
int mod_mpmm_test_main(void)
{
    ...
    //RUN_TEST(utest_mpmm_process_notification_func_domain_ctx_null);
